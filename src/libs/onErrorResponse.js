...
const DATA_TRUNCATED_SQL_ERRNO = 1265;

export default function onErrorResponse(res, error) {
  if (error.errno === COMMAND_DENIED_SQL_ERRNO) {
    res.status(403);
  } else if (
    error.errno === NO_REFERENCED_ROW_SQL_ERRNO ||
    error.errno === USER_DEFINED_EXEPTION_SQL_ERRNO ||
    error.errno === DATA_TRUNCATED_SQL_ERRNO
  ) {
    res.status(400);
  } else {
    res.status(500);
