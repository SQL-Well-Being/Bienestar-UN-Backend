...
router.get("/convocatorias/periodo/:periodo",  async (req, res) => {
    try {
        const [result] = await callProcedure(
            ...
            [req.params.periodo, (req.query.solo_activas === undefined ? 0 : req.query.solo_activas)]
        );
...
router.get("/convocatorias/periodo/:periodo/participaciones",  async (req, res) => {
    try {

        if(req.session.role === "estudiante" && req.query.est_DNI !== req.session.user.username){
            return res.status(403).json({message: "Cannot query someone elses participation."})
        }
...
router.get("/convocatorias/:con_id", async (req, res) => {
    try {
        const [result] = await callProcedure(
            ...
            [req.params.con_id, (req.query.solo_activas === undefined ? 0 : req.query.solo_activas)]
        );

        if(result[0].length !== 0){
            res.json(result[0][0]);
        } else {
            res.status(404).json({message: `No${req.query.solo_activas === '1' ? ' active ' : ' '}convocatorias with id ${req.params.con_id}`});
        }
...
router.post("/convocatorias/:con_id/participaciones",  async (req, res) => {
    try {
        ...
        if(req.session.user.role === "estudiante" && req.session.user.username !== DNI){
            return res.status(403).json({message: "Not allowed to register a participation for someone else"});
        }
