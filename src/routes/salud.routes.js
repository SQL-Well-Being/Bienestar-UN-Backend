...
router.delete("/citas-individuales/:id", async (req, res) => {
    try {
        ...
        if(req.session.user.role === "estudiante"){
            ...
            if(eventResult[0][0].cit_est_per_DNI !== parseInt(req.session.user.username)){
...
router.get("/perfiles-de-salud/estudiante/:DNI", async (req, res) => {
    try {
        if (req.session.user.role === "estudiante" && req.session.user.username !== req.params.DNI) {
            return res.status(403).json({message: "Not alllowed query someone elses information."});
        }
        ...
        if (result[0].length !== 0) {
            ...
            res.status(404).json({message: `No students with DNI ${req.params.DNI}`});
        }
    } catch (e) {
        onErrorResponse(res, e);
    }
...
router.get("/incapacidades", async (req, res) => {
    try {
        if(req.session.user.role === "estudiante" && req.session.user.username !== req.query.est_DNI){
            return res.status(403).json({message: "Not alllowed query someone elses information."});
        }
